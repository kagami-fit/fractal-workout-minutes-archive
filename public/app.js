  ...
  return `
    <section class="insight-panel">
      ...
      <table class="action-table">
        ...
        <tbody>${rows || `<tr><td data-label="担当">未設定</td><td data-label="タスク">未記載</td><td data-label="期限">未設定</td><td data-label="状態"><span class="status-badge">未設定</span></td></tr>`}</tbody>
      </table>
    </section>
  `;
}
...
function keywordPanel(items = []) {
  const body = (items || [])
    .map((item) => `<span class="keyword-chip">${escapeHtml(item.term)}${item.count ? ` ${escapeHtml(item.count)}` : ""}</span>`)
    .join("");
  return `
    <section class="insight-panel">
      <h3>キーワード</h3>
      <div class="keyword-list">${body || `<span class="keyword-chip">未記載</span>`}</div>
    </section>
  `;
}
